# -*- coding: utf-8 -*-
"""
# https://gitlab.gwdg.de/orca-helpers/orca-helpers/-/blob/master/orca_helpers/output_files/print_ir/orca-ir.py
...
parser.add_argument(
    "-n", "--nosave", default=1, action="store_false", help="do not save the spectra"
)

# change line with (integer) for line broadening, default is 15
parser.add_argument(
    "-w", "--linewidth", type=int, default=15, help="line width for broadening"
)
...
for index, freq in enumerate(freqlist):
    ...
    if show_single_gauss:
        ax.plot(
            plt_range_x,
            gauss(intenslist[index], plt_range_x, freq, w),
            color="grey",
            alpha=0.5,
        )
    ...
    if label_peaks:
        ...
        for index, txt in enumerate(peaks):
            ...
            ax.annotate(
                ...
                rotation=90,
                size=6,
                xytext=(0, label_rel_pos_y + corr_factor),
...
if transm_style:
    plt.ylim(
        max(plt_range_gauss_sum_y) + max(plt_range_gauss_sum_y) * 0.1, 0
    )  # +10% for labels
else:
    plt.ylim(
        0, max(plt_range_gauss_sum_y) + max(plt_range_gauss_sum_y) * 0.1
    )  # +10% for labels
...
if show_grid:
    ax.grid(
        True, which="major", axis="x", color="black", linestyle="dotted", linewidth=0.5
    )
